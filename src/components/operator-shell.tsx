...
export function OperatorShell({ title, eyebrow, children }: { title: string; eyebrow: string; children: ReactNode }) {
  ...
  return <div className="operator-app slush-workspace">
    <div className="workspace-ribbon"><span>YOUR PAYMENTS. BACK ON TRACK.</span><span>VERIFY. RECOVER. REPEAT.</span><span>YOUR PAYMENTS. BACK ON TRACK.</span></div><header className="operator-topbar"><Link className="brand" href="/command-center" aria-label="RecoveryOS Command Center"><span className="brand-mark"><Icon name="loop" /></span><span className="brand-copy"><strong>RecoveryOS</strong><small>Payments, unstuck.</small></span></Link><nav className="top-nav" aria-label="Primary navigation">{links}</nav><div className="topbar-actions"><form className="global-payment-search" action="/payments" method="get" role="search"><Icon name="search" /><label className="sr-only" htmlFor="global-payment-search">Find a payment</label><input ref={searchRef} id="global-payment-search" name="q" placeholder="Find a payment…" autoComplete="off"/><kbd>Ctrl K</kbd></form><StatusBadge tone="success" className="header-mode">Test Mode</StatusBadge><Link className="header-recovery-cta" href="/recovery">Recover <Icon name="arrow" /></Link><form action="/api/auth/logout" method="post"><button className="icon-button sign-out" title="Sign out">Sign out</button></form><button className="topbar-menu icon-button" onClick={() => setMobileOpen(value => !value)} aria-label="Toggle navigation" aria-expanded={mobileOpen}><Icon name={mobileOpen ? "close" : "menu"} /></button></div></header>
