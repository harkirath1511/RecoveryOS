...
import { paymentAttempts, paymentJourneys, recoveryOutcomes, recoveryTokens, recoveryWorkflows } from "@/db/schema";
import { requireOperator } from "@/lib/auth/session";

const querySchema = z.object({
  page: z.coerce.number().int().min(1).default(1), pageSize: z.coerce.number().int().min(10).max(100).default(25),
  query: z.string().trim().max(220).optional(), state: z.string().trim().max(80).optional(), provider: z.string().trim().max(120).optional(), method: z.string().trim().max(80).optional(), device: z.string().trim().max(80).optional(), errorCode: z.string().trim().max(120).optional(), workflow: z.string().trim().max(80).optional(), outcome: z.string().trim().max(80).optional(),
...
export async function GET(request: Request) {
  ...
  try {
    ...
    const errorCodeSearch = input.errorCode ? `%${input.errorCode.replace(/[\\%_]/g, "\\$&")}%` : undefined;
    const conditions = [
      ...
      errorCodeSearch ? sql`exists (select 1 from ${paymentAttempts} where ${paymentAttempts.journeyId} = ${paymentJourneys.id} and ${ilike(paymentAttempts.errorCode, errorCodeSearch)})` : undefined,
      ...
      search ? or(
        ilike(sql<string>`cast(${paymentJourneys.id} as text)`, search), ilike(paymentJourneys.razorpayOrderId, search), ilike(paymentJourneys.providerPaymentId, search),
        ilike(sql<string>`cast(${paymentJourneys.state} as text)`, search), ilike(paymentJourneys.provider, search), ilike(paymentJourneys.paymentMethod, search), ilike(paymentJourneys.deviceCategory, search), ilike(latestWorkflowStatus, search), ilike(latestOutcomeCategory, search),
        sql`exists (select 1 from ${paymentAttempts} where ${paymentAttempts.journeyId} = ${paymentJourneys.id} and (${ilike(paymentAttempts.providerPaymentId, search)} or ${ilike(paymentAttempts.providerOrderId, search)} or ${ilike(paymentAttempts.errorCode, search)} or ${ilike(sql<string>`cast(${paymentAttempts.metadata} as text)`, search)}))`,
        sql`exists (select 1 from ${recoveryTokens} where ${recoveryTokens.journeyId} = ${paymentJourneys.id} and ${ilike(recoveryTokens.paymentLinkId, search)})`,
      ) : undefined,
    ];
