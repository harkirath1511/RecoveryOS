...
export async function POST(request: Request) {
  try {
    ...
    await database.transaction(async tx => { const [decision] = await tx.insert(recoveryDecisions).values({ journeyId: journey.id, action: "CREATE_PAYMENT_LINK", policy: "RULES", predictedSuccess: "0", expectedRecoveryAmount: journey.outstandingAmount, safety }).returning({ id: recoveryDecisions.id }); await tx.insert(auditEntries).values({ entityType: "DECISION", entityId: decision!.id, eventType: "RECOVERY_LINK_CREATED", evidence: { safety, journeyId: journey.id, referenceId: input.referenceId, amount: journey.outstandingAmount } }); await tx.insert(recoveryTokens).values({ token, paymentLinkUrl: link.short_url, expiresAt }); });
