...
export async function POST(request: Request) {
  try {
    ...
    try { await database.transaction(async tx => { await tx.execute(sql`select set_config('recovery.max_automated_actions', ${String(env.MAX_AUTOMATED_RECOVERY_ACTIONS)}, true)`); const [decision] = await tx.insert(recoveryDecisions).values({ journeyId: journey.id, action: selection.ranking.action, policy: "LINUCB", policyVersion: selection.version, policyFeatureSchema: policyFeatureSchemaVersion, policyContext: selection.context, candidateActions: allowedActions, policyEstimates: selection.rankings, decisionReason: "Highest LinUCB score among safety-permitted actions.", predictedSuccess: String(selection.ranking.predictedSuccess), expectedRecoveryAmount: selection.ranking.expectedRecoveryAmount, safetyContext, safety: { selected: safety, results: safetyResults } }).returning({ id: recoveryDecisions.id }); const tokenDigest = recoveryTokenDigest(token); const [workflow] = await tx.insert(recoveryWorkflows).values({ journeyId: journey.id, decisionId: decision!.id, action: "CREATE_PAYMENT_LINK", status: "EXECUTED", customerTokenDigest: tokenDigest, externalResourceId: link.id, attemptCount: 1, executedAt: new Date(), expiresAt }).returning({ id: recoveryWorkflows.id }); await tx.insert(auditEntries).values({ journeyId: journey.id, decisionId: decision!.id, entityType: "DECISION", entityId: decision!.id, action: "CREATE_PAYMENT_LINK", reason: "Safety-permitted LinUCB selection", eventType: "RECOVERY_LINK_CREATED", evidence: { safety, safetyResults, journeyId: journey.id, referenceId: input.referenceId, amount: journey.outstandingAmount, paymentLinkId: link.id, policyVersion: selection.version, policyContext: selection.context, selectedAction: selection.ranking.action, rankedActions: selection.rankings, workflowId: workflow!.id } }); await tx.insert(recoveryTokens).values({ tokenDigest, journeyId: journey.id, decisionId: decision!.id, paymentLinkId: link.id, paymentLinkUrl: link.short_url, expiresAt }); }); } catch (error) { await cancelRazorpayPaymentLink(link.id).catch(() => undefined); if (error instanceof Error && error.message.includes("AUTOMATED_ACTION_LIMIT")) await recordDuplicatePrevention(database, journey, "AUTOMATED_ACTION_LIMIT", { safetyContext, racePrevented: true }); throw error; }
